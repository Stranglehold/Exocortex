...
Position: After history_output is populated by prepare_prompt(), before final LLM prompt assembly.

IMPORTANT: This extension MUST run in message_loop_prompts_after, NOT before_main_llm_call.
prepare_prompt() assembles the final LLM prompt from loop_data.history_output, then
before_main_llm_call fires after the prompt is already built. Any history_output
modifications in before_main_llm_call are silently discarded (overwritten next turn).
message_loop_prompts_after fires inside prepare_prompt() after history_output is set —
modifications here reach the model.

Compresses or removes stale tool outputs in loop_data.history_output before each LLM
call. Based on the SleepGate finding (arXiv 2603.14517): stale KV cache entries don't
just waste space — they actively compete with current information, degrading retrieval
accuracy.

This extension operates at the context layer (pre-token), complementing the
attention-level PI mitigation that requires llama.cpp access. Both layers are needed;
this is Layer 1.
