...
export function _drawMessage({
  ...
  if (content && content.trim().length > 0) {
    if (markdown) {
      ...
      processedContent = extractArtifacts(processedContent);
...
// ─── Artifact extraction ──────────────────────────────────────────────────
//
// Strips ```artifact[:type][ title]\n...\n``` fences from markdown content
// before rendering, and dispatches each one to the artifact panel.
// Supported types: html (default), svg, js
//
function extractArtifacts(text) {
  // Matches:  ```artifact           (type=html, no title)
  //           ```artifact:svg       (type=svg, no title)
  //           ```artifact:html My Chart  (type=html, title="My Chart")
  const artifactRe = /^```artifact(?::(\w+))?(?:[ \t]+([^\n]+))?\n([\s\S]*?)^```/gm;
  let found = false;

  const cleaned = text.replace(artifactRe, (_match, type, title, body) => {
    found = true;
    if (window.artifactPanel) {
      window.artifactPanel.update(
        body.trimEnd(),
        type || 'html',
        title ? title.trim() : 'Artifact'
      );
    }
    return ''; // remove from chat display
  });

  return found ? cleaned.trim() : text;
}
