...
def execute_skill(params):
    ...
    def review_workdir_artifacts():
        """Review recent research artifacts from workdir."""
        ...
        if not md_files:
            return None
        file = random.choice(md_files)
        log(f"Reviewing artifact: {file.name}...")
    ...
    while True:
        ...
        if activity_roll < 0.25:
            ...
            # Review recent workdir artifacts (15%)
            artifact = review_workdir_artifacts()
            if artifact:
                log(f"Reviewed: {artifact['title']}")
