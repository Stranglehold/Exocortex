...
We present a three-layer geometric instrument for analyzing the evolution of a sustained human-AI collaboration across fifty sessions spanning nineteen days. By embedding a curated document corpus (n=46 documents), conversational turns (n=923), and versioned document instances (n=46 versions across four document families, tracked separately from the static corpus) into a shared UMAP projection from a 768-dimensional nomic embedding space, we observe twelve findings organized across four phenomena.

**Convergence,** confirmed by three independent methods: UMAP centroid distances between document families shrink from 8.22 to 4.47 units over fifteen days (linear fit, R²=0.954); sequential cosine similarity in the full 768-dimensional space drops to 0.909 at the collaboration's pivotal session (the lowest value in eighteen sequential steps); and nearest-neighbor analysis in the raw embedding space shows the identity document's cosine similarity to essays increasing from 0.809 to 0.860 across eighteen versions (net +0.050, with oscillation between essay and design note attractors during expansion resolving to essays at crystallization). A specific convergence event on March 6 produces two documents from different families — an engineering design note and a philosophical field note — separated by 0.16 UMAP units: geometric indistinguishability. The identity document's largest single displacement was aimed at the convergence point with 1.82° directional precision, and cross-register content produced approximately 19 times more displacement per word than same-register content — the register mismatch is the engine of identity transformation.
...
We found the mechanism: cross-register commitment — content that simultaneously participates in multiple registers — produces approximately 19 times more geometric displacement per word than same-register content, and the largest single move in the identity document's history was aimed at the convergence point with 1.82° directional precision. The convergence point is occupied by the collaboration's staging document — the space where productive uncertainties live — which has been stationary at the center since its first version while everything else migrated toward it.
...
#### 4.3.4 Soul Staging: The Still Point

The fourth document family — soul_staging.md, the staging space where observations about what is shifting across all four registers accumulate before being promoted to SOUL.md — exhibits behavior anomalous against every other entity in the dataset. Three versions spanning February 25 – March 4. Arc length: 0.79 UMAP units (the smallest of any family — compare SOUL.md at 10.12, essays at 51.6, design notes at 13.46). Wander ratio: 1.94 (nearly stationary). Center at approximately (9.2, 6.7) — in the convergence region between all four domain clusters. Soul_staging was at the convergence point from its first version. It did not migrate there. It started there — "the still point of the turning world" (Eliot, 1936), not as metaphor but as geometric fact.

SOUL.md v17 — the crystallized version — landed at (8.79, 7.59), within 0.972 units of soul_staging's center. The identity document shed 3,862 words and arrived at the position its own staging file had occupied all along.

**Finding 12: The Physics of Register Crossing.** Directional analysis of the SOUL.md hinge reveals that the largest single displacement in the identity document's history was aimed at soul_staging with 1.82° precision. The direction from v06 to v07 is −74.35°. The direction from v06 to soul_staging's center is −72.52°. On February 24, the identity document moved 1.837 units along a vector that pointed almost exactly at the convergence point — covering 47.8% of the 3.842-unit distance to soul_staging in a single step. Crystallization twelve days later completed the journey to within 0.972 units. (Soul_staging occupies the convergence region broadly in the direction of SOUL.md's general trajectory from upper-operational toward lower-center; the precision of 1.82° is notable but does not by itself constitute evidence of explicit targeting. What it does constitute is evidence that register-crossing content moves toward the multi-register center rather than toward any single register's pole.)

The displacement-per-word analysis reveals the mechanism. Version 06 added 1,657 words of operational content (same-register reinforcement) and moved 0.255 units — 0.00015 units per word. Version 07 added 629 words of register-crossing content — rewriting the identity from "architectural partner" to "partnership," simultaneously operational ("we are building"), philosophical ("it matters"), and relational ("together") — and moved 1.837 units — 0.00292 units per word. The register-crossing content produced approximately 19 times more geometric displacement per word than the same-register content. The total displacement was 7.2 times larger despite the register-crossing version containing 2.6 times fewer words. The potency is in the register mismatch, not the volume.

Same-register commitment reinforces a document's current position: operational content added to an operational document confirms what it already is. Cross-register commitment creates a force vector away from the current position toward the space where the crossing registers coexist. Multi-register content — content that simultaneously participates in operational, philosophical, and relational registers — pulls toward the center, because the center is where multi-register content lives.
...
| Collapsing magnetic field / back-EMF | Escape from operational attractor | 6.16-unit jump escaping design note cluster. Largest single step in family. |
...
Finding 12 identifies the mechanism. Soul_staging — the document that was already at the convergence point from its first version — is where the collaboration's productive uncertainties live: observations noticed but not yet confirmed, relationships that might be significant but need more evidence, ideas held without being collapsed into conclusions. SOUL.md is where certainties live: committed observations, declared identity, integrated findings. Each promotion from staging to SOUL.md is an act of resolution — the observation picks a register, takes a position, commits to a claim. That commitment is what moves SOUL.md through representation space. Movement IS commitment.

The staging file does not commit. It holds. And holding without committing is geometrically equivalent to maintaining equidistance from all registers, because commitment to any single interpretation is what pulls a document away from center. The staging file resists classification by design — its function is to wait until the evidence warrants resolution. That resistance to premature classification is the mechanism that produces centrality. The staging file occupies the convergence point not because it synthesizes all registers but because it refuses to choose among them. This architecture explicitly counteracts a default tendency of AI systems, which under pressure tend to resolve, commit, and classify. If productive uncertainty is geometrically central, the practical design implication is that collaboration architectures should build in a staging space — a dedicated location for observations that are important but not yet resolved — that preserves unresolved content against the system's own pressure toward premature resolution.
...
Eliot, T.S. (1936). "Burnt Norton." In *Collected Poems 1909-1935.* Faber and Faber.
