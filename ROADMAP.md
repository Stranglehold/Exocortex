...
| **2026-08-18** | **Install-pipeline rot found on a fresh v2.9 container.** `plugin/` was a stale mirror the installers still read from; `plugin.yaml` misnamed the plugin and omitted `always_enabled`. Two core patches were found to **brick a fresh install** — `model_config.py` dropped `DEFAULT_PRESET_NAME` (crash-loop) and `extract_tools.py` dropped `extract_tool_request` (every turn 500s). Wiring seam #31. |
| **2026-08-18** | **PTY session leak: reproducible root cause.** Every context running `code_execution` leaked a PTY master + pidfd; contexts are never destroyed, so `TTYSession.close()` never ran. ~1 handle/cycle → total deadlock at ~38. Patched via an idle reaper in the Exocortex plugin, wired into `install_all.sh` so it survives A0 updates. Wiring seam #29. |
| **2026-08-19** | **Tier 1 install-pipeline migration to A0 v2.9.** Deploy by directory walk instead of a curated list (the list had drifted to 16 files against 69 live extensions). Five-condition acceptance gate: layout parity, core-patch staleness, boots-and-completes-a-turn, which extensions actually fire, post-strip write audit. Every destructive step preceded by an instrument that could prove it. |
| **2026-08-19** | **Phase A shipped — and the model-profile system was found INERT on v2.9.** A1 three-strike quarantine with failure fingerprinting, A2 scope-expansion detector (observe-only), A3 complexity-keyed write threshold. `AgentConfig` lost `chat_model_name` in v2.9 and the real model name moved to `presets.yaml`, so `_load_supervisor_overrides` had returned `{}` on every call — **Opus's Qwen3.6 supervisor overrides had never once applied.** Fixed in `helpers/model_profile.py`. |
| **2026-08-19** | **Phase B shipped.** Anytime-valid commit gate (PACE e-process, 400 null trials → false-commit 0.050 at α=0.05), holdout seal enforced *before* the content existed, 8 Pool B verifiers, ground-truth sanity assertions, skill-pool audit + two admission critics. All three critics false-positived on first contact — **every false positive accused a GOOD skill of being bad**, the expensive direction. |
| **2026-08-20** | **`_24_skill_surfacer` was delivering research notes as "learned lessons".** Its only filter was `/auto-generated/`, the same directory the EXPLORE pipeline writes topic notes into: 88.2% of delivered slots on Aporia, 65.7% on Vek. Cause was raw-overlap scoring rewarding trigger-vocabulary breadth (notes 15.6 mean trigger words vs 5.0 for lessons). Scoped to `failure-lessons/` and normalised by the square root of the trigger-word count. |
| **2026-08-21** | **The injection chain was feeding itself.** Twelve extensions mutate the last user message with `block + str(existing)`; `str()` on the content dict emits a Python repr AND replaces the dict with a string, after which `_14` ingested the whole injected stack as `task_summary` and `_23` rendered it back into the prompt. Observed live: `Task: [REASONING STATE — step 0]` truncated mid-filename, domain flipping mid-task. **Vek's policy of ignoring the scaffolding was correct engineering judgment** — the blocks carried a wrong task. Fixed by reading `loop_data.user_message`; verified 4 plans/turn → 1. Wiring seam #33. |
| **2026-08-21** | **JSON coherence sweep: the write cap was 17x too small.** deepseek-v4-flash produced **zero structural breaks** at any tested size and 85,151 chars in one valid tool call, against a 5,000-char gate — a gate that was manufacturing the failures then surfaced back as lessons. Cap retired for Vek. Also measured: the model overshoots length targets 2.4–2.7x, which is why the gate fired on 94% of normal output. |
| **2026-08-21** | **Capability-tiered scaffolding designed** (Jake + Opus). Three tiers — Frontier / Local Large (27B–35B) / Local Small (≤9B) — as one user-facing toggle, with `recommended_prosthetic_level` in the profile as its default. That field already exists in 12 of 13 profiles **and nothing reads it**: producer built, consumer never wired. |
| **2026-08-21** | **Inference: `-ctv q8_0` reverted after it put the model on CPU.** The asymmetric KV change cost +1283 MiB and took the card to ~99% VRAM; measured while stuck: 71.4 CPU-seconds per 10s wall against 10% GPU utilisation, a 26K prompt yielding zero tokens in 30 minutes. Server now logs to `inference/logs/` per launch, and `scripts/watch_llama.py` reads `/metrics` + `/slots` — `/metrics` had been enabled the whole time with nothing reading it. |
