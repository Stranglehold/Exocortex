...
Reads:
  - /a0/usr/plugins/_exocortex/config/ontology_config.json
...
ONTOLOGY_DIR = "/a0/usr/ontology"  # ontology DATA (per-deployment)
ONTOLOGY_CODE_DIR = "/a0/usr/plugins/_exocortex/helpers/ontology"  # ontology CODE (shipped in plugin)
...
async def _run_queue_resolution(agent, db, config: dict) -> int:
    ...
    try:
        ...
        sys.path.insert(0, ONTOLOGY_CODE_DIR)
        import importlib.util
        spec = importlib.util.spec_from_file_location(
            "resolution_engine",
            os.path.join(ONTOLOGY_CODE_DIR, "resolution_engine.py")
        )
        if spec is None:
            return 0
        ...
        spec2 = importlib.util.spec_from_file_location(
            "ontology_store",
            os.path.join(ONTOLOGY_CODE_DIR, "ontology_store.py")
        )
        if spec2:
...
def _update_relationship_confidence() -> int:
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            ...
            os.path.join(ONTOLOGY_CODE_DIR, "relationship_extractor.py")
        )
        if spec is None:
            return 0
        module = importlib.util.module_from_spec(spec)
