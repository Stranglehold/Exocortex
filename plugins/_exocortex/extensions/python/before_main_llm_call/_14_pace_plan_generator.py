...
#
# WHY THIS IS LOAD-BEARING (live-verified 2026-08-20, VekV2 probe + driven turns):
# _22/_23/_24 all mutate the last user message with
#     user_msg["content"] = block + "\n\n" + str(existing)
# When `existing` is the content DICT, str() yields a Python repr AND replaces the
# dict with a string. _extract_message_content returns the whole string for str
# content, so from the second iteration on this function receives the entire
# injected stack as "the task". _create_plan then stored it as task_summary and
# _23 rendered it straight back into the prompt. Observed live:
#
#     [PACE PLAN — ACTIVE]
...
#
# The Task field WAS the previous turn's injection, truncated mid-filename. Vek
# consequently learned to treat the whole scaffolding as untrusted noise, which was
# correct behaviour on its part - the blocks carried a wrong task, an unstable
# domain and stale artifacts.
#
# The strip existed before this, but was wired ONLY to the hash (change detection),
# and its pattern list covered PACE PLAN + MODEL CONFIGURATION while missing the
# three block types that actually leaked.
#
# Headers are matched as bracket-delimited tokens rather than on the em-dash, so a
# text-encoding difference cannot silently turn this gate into a no-op.
# Terminators verified against the live emitters:
#   _23_pace_plan_injector : "[PACE PLAN ...]" ... "[/PACE PLAN]"      (closed tag)
#   _22/_13 reasoning state: ... "Do not retry approaches listed in Tried."
#   _22/_13 artifacts      : ... "These files exist on disk. Check them before rebuilding."
#   _24_skill_surfacer     : header + consecutive "- " lines
_INJECTED_BLOCK_PATTERNS = [
    # PACE first: its own Task: field can contain the other headers verbatim.
    r"\[PACE PLAN\b.*?\[/PACE PLAN\]",
    r"\[MODEL CONFIGURATION\].*?EI active.*?\n",
    r"\[REASONING STATE\b[^\]]*\].*?Do not retry approaches listed in Tried\.",
    r"\[ARTIFACTS\b[^\]]*\].*?These files exist on disk\. Check them before rebuilding\.",
    r"\[LEARNED LESSONS\b[^\]]*\](?:\r?\n-[^\n]*)*",
]

# Last-resort cleanup: a block whose terminator is missing (truncated upstream)
# would otherwise survive. Remove the HEADER LINE ONLY - never span to end-of-string,
# because the genuine user text is what follows and eating it is the expensive error.
_ORPHAN_HEADER_RE = re.compile(
    r"^[ \t]*\[(?:PACE PLAN|/PACE PLAN|REASONING STATE|ARTIFACTS|LEARNED LESSONS|"
    r"MODEL CONFIGURATION)\b[^\]]*\][ \t]*\r?\n?",
    flags=re.MULTILINE,
)


def _strip_injected_blocks(content: str) -> str:
    """Remove our own injected blocks from a message before treating it as the task.

    Returns the operator's actual text. Never returns None; on any failure the
    original content is passed through unchanged (graceful degradation - a
    contaminated task is bad, no task at all is worse).
    """
    if not content:
        return ""
    try:
        ...
        clean = _ORPHAN_HEADER_RE.sub("", clean)
        clean = re.sub(r"\n{3,}", "\n\n", clean).strip()
        # If stripping consumed everything, the input was pure injection: prefer the
        # original over an empty task so the planner still has something to hash on.
...
def _hash_message(content: str) -> str:
    ...
    clean = _strip_injected_blocks(content)[:500]
