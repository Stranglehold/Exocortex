...
Start: launched by the _00_idle_watch_bootstrap extension (before_main_llm_call),
       which spawns this daemon detached (start_new_session) and keeps it alive
       via a pidfile guard. Shipping the launcher as a plugin extension — rather
       than an /etc/supervisor conf — makes the idle engine clone-and-go: the
       daemon runs wherever the plugin is dropped, and survives A0 image rebuilds.
Logs:  /a0/usr/workdir/workspace/office/idle_watch.log  (or `docker logs <ctr> | grep IDLE-WATCH`)
"""
