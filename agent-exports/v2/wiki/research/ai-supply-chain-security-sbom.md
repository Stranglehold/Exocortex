...
**Sigstore-backed model signing** — cryptographic attestation of model weights at release time, enabling downstream consumers to verify provenance before deployment. Aligns with CISA guidance on software supply chain security.

**ML-BOM / AI-BOM standards** — CycloneDX 1.7 and OWASP AIBOM extend traditional SBOM schemas to capture AI-specific lifecycle stages: model weights, training epochs, data lineage, and evaluation metrics.

**Pipeline attestation** (arXiv 2603.28988, Mar 2026): Verifiable training and release provenance through cryptographic attestation of training runs. Addresses the gap between software supply-chain guarantees and AI-specific provenance.

### Detection & Mitigation

**Poisoning detection** — Trimmed Mean and Secure Aggregation (HE-based SecAgg) for federated learning settings; statistical outlier detection on training loss curves.

**Backdoor detection** — Activation patching and fine-tuning neutralization for pre-trained models with embedded triggers.

**Agent skill verification** — SLSA-based provenance and signing frameworks for AI modules; maturity model L1-L4 from basic provenance to formal verification (arXiv 2603.02512, May 2026).
...
Tertiary insight: The emergence of skill-level supply chain attacks (PyPI/npm/MCP) means AI security must extend beyond model provenance to cover the entire agent ecosystem — tools, skills, and interaction data all require attestation.
