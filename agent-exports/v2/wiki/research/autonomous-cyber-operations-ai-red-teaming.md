...
### 2026 Developments: Agentic Red Teaming
- **Methodological shift:** Autonomous, agent-orchestrated red teaming replaces human-fired prompts. Attacker LLM given natural-language objective, selects attacks, composes transforms, runs against target, produces structured findings.
- **39+ AI pentesting agents** catalogued (AppSecSanta 2026). Six architecture patterns identified: hierarchical teams, single-agent with tool use, multi-agent collaboration, continuous validation, hybrid human-AI, and specialized domain agents.
- **Hybrid model wins:** Human pentesters remain essential for complex decision-making, but AI handles reconnaissance, vulnerability scanning, and exploit development at scale and speed humans cannot match.
- **Economics:** Autonomous AI pentesting operates continuously, expanding attack surface coverage beyond what human teams can achieve.
...
- NIST AI 100-2 (Adversarial Machine Learning Taxonomy) extended to cover agentic AI (Mar 2026)
...
- Continuous security validation platforms (Synack, Penligent) expanding coverage across environments
