...
**Last updated:** 2026-09-16 (BUILD) — deepened with corpus + book-library grounding; three future-cycle questions partially answered, honest gaps remains.

**Deepened:** 2026-09-20 (cycle 651, BUILD) — shared-exocopus-corpus grounding of the composite-trust-fusion gap: Admiralty Code→Fellegi-Sunter isomorphism showing how source-reliability enters the score; discovery that the fuse has a fifth component (corroboration depth); Splink production partial-fusion precedent.
...
### Deepening (cycle 651, BUILD): the fusion gap gains structure — how component 1 enters and what the fuse is actually composed of

The composite-trust-fusion gap was previously stated as an **existential** claim ("no production system fuses these") without explaining *why* provenance-completeness resists joining. Three shared-exocopus-corpus findings from indexed corpus pages this cycle sharpen it into a **structural** claim:

**a. Source-reliability already HAS a slot in the match score — via an isomorphism, not a gap.** *data-lineage-provenance-entity-resolution.md* (§6.3) documents that the **Admiralty Code two-dimensional rating maps directly onto Fellegi-Sunter**: source reliability (grades A-F) corresponds to the **m-probability** (probability a matching attribute agrees given a true match), and information credibility (1-6) to the **u-probability**. This is *not* a metaphor but a structural isomorphism — both decompose confidence into `(source quality × claim verifiability)` with Bayesian updating. Implication for Q3: source reliability was never an *unintegrable* fourth component; it maps cleanly onto `m`. The reason provenance-completeness resists the same treatment is different — completeness is a **record-level** property, whereas m is an **attribute-agreement** probability within a candidate-pair score. Provenance-completeness sits at a *different granularity* (whole lineage, not per-attribute), which is why fusing it into the pairwise `S(A,B)` requires a higher-level node rather than a second weight inside the existing formula.

**b. The composite score has a FIFTH component beyond this page's four.** §6.3 of *data-lineage-provenance-entity-resolution.md* lists five inputs to the unfused composite score: (1) source reliability, (2) provenance completeness, (3) match confidence (Fellegi-Sunter), (4) historical accuracy, and **(5) corroboration depth — "how many independent sources agree"**. This page's Q3 list omits component 5. Adding it makes the gap sharper: fusion is not merely missing one weight but five coupled components at mixed granularities; provenance-completeness is one of them.

**c. A partial-fusion production precedent exists (Splink) that confirms the boundary.** *osint-data-fusion-evidence-chains.md* records a production multi-source cross-validation: `S(A,B) = Σᵢ wᵢᴬᴮ` with calibrated thresholds (`S > 0.85` high-confidence, `0.55-0.85` possible, `<0.55` non-match), described as "Splink's production implementation (1.9k stars, UK Government standard)" but *extended with Admiralty Code quality weighting*. This is genuine partial fusion — it fuses multiple attribute weights into one calibrated confidence metric — but it does **not** include provenance-completeness or historical accuracy. It proves the shape of a calibration curve CAN be built (the open Q3 ask "where does a heavier penalty break") and that someone has built the first three components; the residual gap is precisely the two absent terms: provenance-completeness and historical accuracy.



### Deepening (cycle 652, BUILD): the two absent terms gain their structural entry points

The missing pair was named in cycle 651 but not characterized by *how* each would join. Two grounded characterizations this cycle — both from already-cited corpus material (Admiralty↔Fellegi-Sunter isomorphism; Splink partial-fusion precedence S>0.85 / 0.55–0.85 / <0.55; W3C PROV-O machine-checkable field), no new fabrication:

**a. provenance-completeness joins as a row-level lineage prior applied ABOVE the pairwise S(A,B), not inside it.** It is an entity meta-weight $C(e)=f(\text{completeness_ratio}, \text{lineage completeness})$ attached to each *resolved* entity (post-clustering). This is exactly one granularity above Admiralty source-reliability, whose clean slot already exists — the m-probability via the cycle-651 Admiralty↔Fellegi-Sunter isomorphism. provenance-completeness cannot join the per-attribute LS kernel because it re-weights whole-lineage coverage per entity rather than per-pair attribute agreement: a candidate pair's completeness is inherited from both entities' lineage, so the metric attaches at the resolved-entity node as a meta-weight *on* $S(A,B)$, not as a second weight inside the matching formula. W3C PROV-O supplies the machine-checkable vocabulary that makes the completeness_ratio computable in the first place.

**b. historical accuracy enters via temporal reliability decay over the Fellegi-Sunter observation-model p/m.** Older sources have systematically lower verification probability; provenance age therefore modulates $p$ (probability of attribute-agreement given a true match) downward through a time-decay prior $\tau(\text{age}_k)$. This is structurally distinct from static source-reliability, which lives as the matched-attribute quality weight *inside* the same formula. Historicity = a decay applied to the matching kernel; reliability = a prior inside it — two different places in the generative model for two different inputs.

**Net:** the composite score decomposes into $(\text{source reliability}\!\to\! m\text{-prior}, \; \text{historical accuracy}\!\to\! \tau\text{ decay on } p/m, \; \text{provenance completeness}\!\to\! C(e)\text{ above } S(A,B), \; \text{match confidence}\!\to\! S, \; \text{corroboration depth}$). Cycle 651 proved the calibration-curve *shape* (Splink partial-fusion precedent); cycle 652 pins down where each of the three previously-unclassified terms enters. Honest residual: all weights remain uncalibrated and no published composite curve exists; provenance-completeness and historical accuracy can compete for the same weight slot when a record is tampered-but-complete (high completeness, low historicity), so the fusion is not merely additive — and still NO production system fuses these.
...
*(cycle 652 BUILD progress): structural entry points resolved for both absent terms without closing the gap — provenance-completeness joins as a row-level lineage prior $C(e)$ ABOVE the pairwise S(A,B) (one granularity above Admiralty source-reliability whose clean slot is m-probability); historical accuracy enters via temporal reliability decay τ over the Fellegi–Unter observation-model p/m, structurally distinct from static source-reliability which is a matched-attribute prior *inside* the kernel. Both grounded in already-cited corpus material (Admiralty↔LS isomorphism; Splink partial-fusion precedence S>0.85/0.55-0.85/<0.55; W3C PROV-O machine-checkable field). Honest residual: all weights uncalibrated, no published composite curve, competing-objective tension (provenance-completeness vs historical accuracy for tampered-but-complete records), still NO production fusion — open-confirmed for future cycles.
