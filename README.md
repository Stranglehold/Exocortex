...
**Idle Time Engine (V2)** — Autonomous self-improvement loop for the deployed agent, triggered when the agent has been idle. V2 replaces the binary WORKSHOP/FIELD cycle selection with a three-phase adaptive system: MAINTAIN (integrity check + sleep consolidation, 15 steps), BUILD (wiki deepening + skill capture, 30 steps), and EXPLORE (research sprint from `interests.md`, 20 steps). Cycle selection is fully deterministic — a counter-based state machine with configurable thresholds, no LLM judgment required. EXPLORE fires on content saturation OR a 5-cycle time cap (OR logic — either condition alone is sufficient). Each cycle closes by calling `cycle_close.py`, which batch-writes the journal entry, office panel feed, and a `cycle_result.json` signal file — the signal file feeds the next cycle selection. Phase 0 of every MAINTAIN cycle runs `integrity_check.py`, which validates the wiki index against the filesystem (missing pages, status mismatches, stale arXiv sources) before doing any other work. Engine state persists across context changes via `engine_state.json`.
...
- `IDLE_TIME_ENGINE_V2_DEFINITIVE.md` — V2 adaptive cycle selection: MAINTAIN/BUILD/EXPLORE, cycle_close batch bookkeeping, integrity check, office panel integration
