...
function intelPanelFactory() {
    return {
        ...
        // Called by x-init when Alpine mounts the component
        async init() {
            try {
                const [td, sd, hd] = await Promise.all([
                    ossApi("/api/topics"),
                    ossApi("/api/staging", "POST", { limit: 1 }),
                    ossApi("/api/health"),
                ]);
                this.topics      = td.topics || [];
                this.newCount    = sd.total  || 0;
                this.claimsTotal = sd.total  || 0;
                this.ossHealth   = hd;
            } catch (e) {
                console.warn("[INTEL-PANEL] background init:", e.message);
            }
        },
