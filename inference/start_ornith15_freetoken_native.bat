@echo off
REM ============================================================================
REM  Ornith-1.5-35B-A3B on NATIVE FreeToken (Windows) -- port 1236
REM
REM  VERIFIED WORKING 2026-08-22 21:31. This file reproduces the exact invocation
REM  that served: 45.3 tok/s, native structured tool_calls, reasoning_content
REM  separated, and BOTH containers reaching it on host.docker.internal:1236.
REM
...
REM
REM  LOAD TIME: ~26 MINUTES. Not a hang.
...
REM    {"status":"ok"}. Do not kill it at 10 minutes. Converting to FTW with the
REM    NATIVE 0.1.1 `ft checkpoint` is what would shorten this -- untried, and it
REM    needs cuda:0 free (i.e. nothing else serving).
REM
REM  PORT 1236, NOT 1235
REM    1235 belongs to llama.cpp (start_ornith15_prod.bat). Keeping them apart
REM    lets both run and be compared. For reference, on identical weights:
REM        llama.cpp    50.3 tok/s, ~4 min load,  vision working ~2.8 s/image
REM        FreeToken    45.3 tok/s, ~26 min load, vision untested here
REM
REM  AGENTS REACH IT AT host.docker.internal:1236 (verified 200 from both
