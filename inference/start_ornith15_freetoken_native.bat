...
REM
REM  --kv-reserve-tokens MUST TRACK --max-seq-len-override. THIS BIT US.
REM    --max-seq-len-override only sets the ADVERTISED context.
REM    --moe-cache-auto is documented "MoE-priority (KV gets
REM    --kv-reserve-tokens as a floor)", so expert slots claim VRAM first and
REM    KV is left with the floor -- default 8192. Result: /v1/models reported
REM    context_length=100000 while the server rejected anything past 8,255:
REM      "prompt is too long: 8904 tokens > 8255 maximum (prompt + generation)"
REM    Hermes read 100000, filled to it, and reported "context length exceeded
REM    (30 tokens) Cannot compress further" -- it could not get under a limit
REM    it was never told about. Raising the override ALONE DOES NOTHING except
REM    make the advertised number lie harder.
REM    Both flags come from %CTX% now so they cannot drift apart. If a large
REM    CTX will not fit, the engine fails at LOAD instead of serving a false
REM    number -- loud beats silent.
REM
...
echo   model=%MODEL%
echo   port=%PORT%  ctx=%CTX%  kv-reserve=%CTX%  moe-backend=%BACKEND%
