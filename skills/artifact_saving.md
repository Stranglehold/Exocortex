...
```
/a0/usr/workdir/artifacts/<slug>.html      ← the artifact itself
/a0/usr/workdir/artifacts/<slug>.json      ← sidecar metadata (optional but encouraged)
```

`<slug>` should be snake_case, URL-safe, and descriptive (e.g. `network_graph`, `portfolio_dashboard`).
...
## Emit + save in one step

After saving to disk, emit the artifact to the panel with the **same title**:

````artifact:html Network Graph
<full html content here>
````

The artifact panel deduplicates by title — the sidebar and the panel will both show the same artifact. If you update the file and re-emit with the same title, the existing tab refreshes in-place.
